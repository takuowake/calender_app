...
// return datasをすることで、PlanListDataNotifierのstateにdatasが格納される
class PlanListDataNotifier extends StateNotifier<List<PlanItemData>> {
  // 初期状態として空のリストを設定
  PlanListDataNotifier() : super([]);


  // データベースから取得したPlanItemDataのリストを非同期に取得しdatasに代入
  Future<List<PlanItemData>> read() async {
    final _db = MyDatabase();
    // データベースから全てのデータを取得し、datasに格納
    List<PlanItemData> datas = await _db.readAllPlanData();
    return datas;
  }
  readData() async {
    final _db = MyDatabase();
    final datas = await _db.readAllPlanData();
    return datas;
  }

  void makeDailyList(DateTime date) async {
    List<PlanItemData> list = [];
    List<PlanItemData> datas = await read();
    for (PlanItemData data in datas) {
...
// PlanListDataNotifierのインスタンスを提供するためのProviderを定義
final planListDataProvider = StateNotifierProvider((ref) => PlanListDataNotifier());
...
class PlanList extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    // planListDataProviderの値を取得するためのProvider
    final planListState = ref.watch(planListDataProvider);

    // planListDataProviderの値を変更・更新するためのProvider
